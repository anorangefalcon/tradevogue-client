...
export class MonetizationComponent {
    ...
    this.dialogService.responseEmitter.subscribe({
      next: (res: any) => {
        if (
          ...
          const itemIndex = this.paymentKeys.findIndex(
            (key: any) => key._id === id
          );
          ...
          this.fetch
            .HTTPPOST(this.util.URLs.deletePaymentKeys, body)
            .subscribe((response: any) => {
              if (response) {
                this.toastService.successToast({
                  title: 'Payment Key Deleted Successfully',
                });
                this.fetchData();
              }
            });
  ...
  fetchData() {
    this.fetch
      .HTTPGET(this.util.URLs.getAllPaymentKeys)
      .subscribe((response: any) => {
  ...
  onSubmitPassword(): void {
    if (this.passwordForm.valid) {
      ...
      try {
        this.fetch
          .HTTPPOST(this.util.URLs.verifyPassword, { password })
          .subscribe(
            (res: any) => {
              if (res === 'okay') {
  ...
  onUpdate() {
    if (this.selectedItem && this.stripeKeysForm) {
      ...
      const enable =
        this.stripeKeysForm.get('enableDropdown')?.value === 'true';
      const id = this.selectedItem._id;
      ...
      this.fetch
        .HTTPPOST(this.util.URLs.updatePaymentKeys, body)
        .subscribe((response: any) => {
          this.fetchData();
  ...
  delete(key: any) {
    ...
    let template: any = {
      ...
      subtitle:
        'The key will be permanently deleted, and recovery will not be possible. Are you sure you want to proceed?',
      type: 'confirmation',
  ...
  onSubmit() {
    if (this.stripeKeysForm.valid) {
      ...
      this.loginCheckService.getUser('token').subscribe((data: any) => {
        ...
        this.fetch
          .HTTPPOST(this.util.URLs.addPaymentKeys, body)
          .subscribe((response: any) => {
            this.fetchData();
            this.stripeKeysForm.reset();
          });
      ...
      this.loginCheckService.getUser('token').subscribe((data: any) => {
        ...
        this.fetch
          .HTTPPOST(this.util.URLs.addPaymentKeys, razorpayBody)
          .subscribe((response: any) => {
