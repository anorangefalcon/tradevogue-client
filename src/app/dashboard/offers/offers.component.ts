...
constructor(
      ...
      this.dialogService.responseEmitter.subscribe({
        next: (res: any) => {
          if (res) {
            ...
            this.fetchDateService.HTTPPOST(this.BackendUrls.URLs.deleteOffer, this.OfferBodyDeleted).subscribe((response:any) => { 
  ...
  deleteAllOffers(){
    ...
    this.OfferBodyDeleted={data:this.deleteList,parameters:this.TemplatePagination};
  ...
  offerSubmit(){
    ...
    if (!this.getOfferImage()) {
      this.toastService.errorToast({title:'Please upload image or if already uploaded than wait until its finish uploading'});
      this.loading=false;
      return;
    }
...
// for offer start date to be today date
  ...
  ActiveStatus(event: any, data:any,index:any,) {
    const body = { data, status: event.target.checked ,parameters:this.TemplatePagination};
    this.allSubscriptions.push(
    this.fetchDateService.HTTPPATCH(this.BackendUrls.URLs.updateOfferStatus, body).subscribe({next:(response:any) => {
      if(!response.data.length){
        this.allOffers=[];
        this.totalCount=0;
        return;
  ...
  DeleteOffer(event:any){
    ...
    this.OfferBodyDeleted={id: this.deleteId,parameters:this.TemplatePagination }
    this.dialogService.confirmationDialogBox(template);
  }
